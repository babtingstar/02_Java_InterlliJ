...
public class FieldEx1 {/*
    ...
    - protected : 같은 패키지에서는 접근 가능 / 다른 패키지는 상속받은 클래스만 가능
    - default : 접근제한자를 작성하지 않을 경우 기본값 / 같은 패키지 안에서만 접근 가능
    - private : 클래스 파일명 안에서만 접근 가능 / 외부 클래스에서 접근하려면 getter, setter 메서드를 사용
     */

    //접근제한자에 따른 필드 선언

    public String 퍼블릭필드 = "공개 필드";
    protected String 프로텍티드필드 = "보호된 필드";
    /*default 생략*/ String 디폴트필드 = "기본 접근 필드";
